...
for i in range(ADC_SAMPLESPERWAVE*ADC_ACWAVESTOREAD):
    ...
    for chan in range(1):
        ...
        response = spi.xfer2([6+((4&chan)>>2),(3&chan)<<6,0,6+((4&chan)>>2),(3&chan)<<6,0,6+((4&chan)>>2),(3&chan)<<6,0],2000000,delay)
        #print(response[4] >>5) 
        print(response)
        #print("{}, {}".format(((response[1] & 15) << 8) + response[2],(response[3] << 4) + (response[4] >> 4)))
